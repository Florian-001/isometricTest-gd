...
| `scenes/friendlies/wizard.tscn` | `wizard` | Wizard 1 | None |
| `scenes/friendlies/cleric.tscn` | `cleric` | Cleric 1 | None |

Warrior and Archer inherit the original run character stats. Wizard and Cleric share `friendly_spellcaster.tres` stats, with an individual scene class assignment; both start with 24 HP and no equipment. Their staff artwork is part of the sprite and imposes no equipment requirement. Cleric unlocks **Heal and Beam at level 1**, then **Focus at level 2**. See [class editing](character_classes.md) for multiclass allocations and the developer bypass.
