...
func _run() -> void:
	if not Engine.is_editor_hint():
		push_error("Run with --editor --script res://tests/run_bandit_editor_tests.gd")
		quit(1)
		return
	await create_timer(3.0).timeout
	while EditorInterface.get_resource_filesystem().is_scanning():
		await process_frame
	for entry in [{"id": "bandit_warrior", "hp": 20}, {"id": "bandit_ranger", "hp": 12}]:
