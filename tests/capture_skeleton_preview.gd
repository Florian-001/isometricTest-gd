...
func _capture() -> void:
	...
	var entries := [
		["skeleton_warrior", TacticalCharacter.Facing.LEFT, "Warrior Left"],
		["skeleton_warrior", TacticalCharacter.Facing.RIGHT, "Warrior Right"],
		["goblin_warrior", TacticalCharacter.Facing.RIGHT, "Existing Goblin"],
		["skeleton_archer", TacticalCharacter.Facing.LEFT, "Archer Left"],
		["skeleton_archer", TacticalCharacter.Facing.RIGHT, "Archer Right"],
	]
	...
	label.text = "Skeleton Warrior: 20 HP / Strike: 6 damage / CR 1\nSkeleton Archer: 12 HP / Enemy Shot: 5 damage / CR 1\nBoth facings shown beside the existing Goblin Warrior at equal artwork scale."
	field.add_child(label)
	await process_frame
	await RenderingServer.frame_post_draw
	var error := root.get_texture().get_image().save_png("res://.godot/skeleton_preview.png")
	print("SKELETON_PREVIEW_SAVED (error %d)" % error)
	field.queue_free()
	await process_frame
	quit(error)
