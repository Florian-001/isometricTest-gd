...
func _test_battle_ui() -> void:
	...
	check(buttons.get_child_count() == 9, "battle bar shows Ram as ninth warrior ability")
	check((buttons.get_child(8) as Button).tooltip_text.contains("No chain push"), "Ram tooltip explains knockback")
