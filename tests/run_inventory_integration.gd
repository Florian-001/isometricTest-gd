...
func _run() -> void:
	...
	_check(inventory.starting_items.size() == 5, "GeneralInventory should expose all configured unused sample items")
	_check(
		inventory.starting_items.map(func(item: ItemDefinition): return item.display_name)
		== ["Iron Sword", "Ranger Armor", "Sage Charm", "Wooden Sword", "Ranger Bow"],
		"the existing items and new Ranger Bow should remain on GeneralInventory without changing order"
	)
	_check(inventory.get_items().size() == 5, "the general inventory should start with five unused sample items")
	main._on_inventory_button_toggled(true)
	_check(screen.visible, "the Inventory button should open the inventory screen")
	_check(screen.general_entries.get_child_count() == 5, "the screen should list every unused item")
	...
	_check(_ability_summary(screen, "Arrow") == "Requires a Ranged weapon", "incompatible abilities should remain visible with a clear reason")
	...
	_check(weapon_button.text.contains("Melee"), "equipped weapons should show their type")
	...
	_check(inventory.get_items().size() == 6, "unequipped gear should return to the general inventory")
	...
	_check(_ability_summary(screen, "Strike") == "Requires a Melee weapon", "unequipping should disable weapon-dependent abilities")
	...
	_check(inventory.get_items().size() == 5, "equipping an item should remove it from the general inventory")
	...
	_check(inventory.starting_items.size() == 5, "runtime equipment transfers must not mutate GeneralInventory's configured starting array")
