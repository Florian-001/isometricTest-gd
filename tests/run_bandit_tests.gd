...
const ENEMIES := [
	{"id": "bandit_warrior", "hp": 20, "constitution": 5, "damage": 6, "weapon": "iron_sword", "attack": "strike", "stat": UnitStat.Type.STRENGTH, "type": ItemDefinition.WeaponType.MELEE},
	{"id": "bandit_ranger", "hp": 12, "constitution": 3, "damage": 5, "weapon": "weathered_bow", "attack": "enemy_shot", "stat": UnitStat.Type.DEXTERITY, "type": ItemDefinition.WeaponType.RANGED},
]
...
func _test_enemy(entry: Dictionary) -> void:
	...
	_check(definition.passive_abilities.is_empty() and definition.starting_class == null, "%s starts without passives or a friendly class" % entry.id)
	...
	for cell in valid_cells:
		...
		var forecast := AIBoardSnapshot.from_battle(units, grid.grid_size)
		planner._forecast_ability(actor, attack, cell, forecast, targeting, actor.get_enemy_ai_profile())
	...
	var scaled_hit := await executor.execute(actor, attack, target.grid_cell, units, grid, targeting, {})
	_check(scaled_hit and scaled_health_before - target.current_health == entry.damage + 3, "%s actual damage scales with its normal attribute" % entry.id)
	actor.clear_dev_stat_override(entry.stat)
	actor.reset_ability_action()
...
func _battle(map: BattleMapDefinition, payload: Dictionary = {}) -> TacticalBattle:
	...
	_check(battle.initialization_succeeded, "Battle initializes with bandit-compatible resources")
	return battle
...
func _test_palette_and_scenarios() -> void:
	...
	_check(ScenarioSaveStore.validate_payload(fresh).ok, "Bandit setup uses the existing scenario schema")
	_dispose_battle(battle)
	await process_frame
	var restored := _battle(map, fresh)
	_check(restored.capture_save_payload(true).setup.units == fresh.setup.units, "Restart preserves bandit setup, equipment, and stat overrides")
	restored._open_dev_mode()
	for index in range(ids.size()):
		var actor := _find(restored, ids[index])
		_check(actor != null and actor.current_health == ENEMIES[index].hp + 4, "Restart applies the bandit's authored Constitution override")
		if actor != null:
			actor.apply_damage(3)
			actor.set_facing(TacticalCharacter.Facing.RIGHT)
	var exact := restored.capture_save_payload(false)
	var directory := "res://.godot/bandit_scenario_tests"
	var saved := ScenarioSaveStore.save_new(exact, directory)
	_check(saved.ok, "Damaged bandit scenario saves to disk")
	var loaded := ScenarioSaveStore.load_save(saved.path, directory) if saved.ok else {}
	_check(loaded.get("ok", false), "Bandit scenario reloads from disk")
	...
	if loaded.get("ok", false):
		...
		for index in range(ids.size()):
			...
			var entry: Dictionary = ENEMIES[index]
			_check(actor != null, "Saved bandit stable ID is restored")
			if actor == null:
				continue
...
func _test_templates() -> void:
	...
	for entry in ENEMIES:
		_check(not original_pool.has(_scene(entry)), "Bandits are opt-in for existing template pools")
		var custom := demo.duplicate() as BattleMapTemplateDefinition
		var stage := RunCombatStage.new()
		stage.starting_cr = 3
		stage.enemy_pool = [_scene(entry)]
		var config := RunConfig.new()
		config.combat_stages = [stage]
		var floor_settings := RunCombatProgression.resolve_floor(config, 1)
		_check(floor_settings.error.is_empty(), "Run stages accept %s in a temporary pool" % entry.id)
		custom.combat_rating = floor_settings.combat_rating
		custom.enemy_pool = floor_settings.enemy_pool
		...
		_check(count == 3, "CR 3 template populates three bandit enemies")
		_dispose_battle(battle)
		await process_frame
